...
export default function ProductReviews({ product }) {
  ...
  useEffect(() => {
    axios.get('/api/orders').then((response) => {
      // console.log(response.data[0].line_items[0].price_data);
      ...
      orders.forEach((order) => {
        ...
        const hasBought =
          ...
          order.line_items.some((item) => {
            return item.price_data.product_data.name === product.title;
          });
        if (hasBought) {
          setBuyed(true);
        }
  ...
  return (
    <div>
      ...
      <ColsWrapper>
        <div>
          <WhiteBox>
            {!buyed && <p>You need to buy this product to leave a review</p>}
            {buyed && (
              <div>
